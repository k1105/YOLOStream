        ...
        if len(relation[i]) == 0:
            ...
            for key, buffered_data in bbox_buffer.items():
                if buffered_data['count'] >= min_frames_for_new_person:
                    new_person = Person(peopleCounts, {'x': 0, 'y': 0}, buffered_data['bbox'], CharData("i", 0, 0, 1, "alphabet_i"), "walking")
            ...
            if person:
                ...
                if person.characterUpdatedFrameCount < 2:
                    person.characterUpdatedFrameCount +=1
                else:
                    person.update_display_character(hitomoji_data, character_data)
                    person.characterUpdatedFrameCount = 0
