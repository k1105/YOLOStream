...
def run_scripts(address, port, mirrored, gpu):

    venv = os.path.join(os.environ['VIRTUAL_ENV'], 'Scripts', 'python.exe')

    # tracker.py を実行時の引数に基づいて実行
    tracker_cmd = [venv, "tracker-gpu.py"] if gpu else [venv, "tracker.py"]
...
if __name__ == "__main__":
    ...
    run_scripts(args.address, args.port, args.mirrored, args.gpu)
