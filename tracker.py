...
while True:
    ...
    if bbox_results and bbox_results[0].boxes is not None:
        bboxes = [
            Bbox(
                float(score.cpu()),
                [float(b) for b in box.cpu().numpy()]
            )
            for box, score, class_id
            in zip(bbox_results[0].boxes.xyxy, bbox_results[0].boxes.conf, bbox_results[0].boxes.cls)
    ...
    if pose_results:
        for person_pose in pose_results:
            if (person_pose.keypoints is not None 
                and person_pose.keypoints.xy is not None 
                and person_pose.keypoints.conf is not None):
                keypoints_xy = person_pose.keypoints.xy.cpu().numpy().tolist()
                keypoints_conf = person_pose.keypoints.conf.cpu().numpy().tolist()
                pose_data.append({
    ...
    people, peopleCounts, bbox_buffer, bufferedBboxCount = update_people(
        relation, people, bboxes, bbox_buffer, peopleCounts, bufferedBboxCount
    )
